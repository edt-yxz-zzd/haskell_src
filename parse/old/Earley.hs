...
{-
    ...
    input:
        main_symbol
        -- nfsm = (initials, finals, transitions, nulls)
        input_nfsm + rule_nfsm ==>> 8 input msg types
    other msg types:
        
    interface types:
        MainSymbolM symbol --< InitialInputStateI
            -- ==>> MainWantedM s symbol
            <<- InitialInputStateM s
        MainWantedM s symbol ==>> LeftWantedM s symbol
        MainWantedM s symbol --< LeftWantedI s symbol
            -- ==>> MainWantedInstanceM s symbol s
            <<- LeftWantedInstanceM s symbol s
            <<- BottomupInstanceM s1 symbol s2
        MainWantedInstanceM s0 symbol s1 --< FinalInputInstanceI s1
            -- ==>> MainWantedTrueInstanceM s0 symbol s2
            <<- FinalInputInstanceM s1 s2
        Final[Input|RP]StateM s1 --< Null[Input|RP]TransitionEndI s1
            -- generate special final instance (s0, s1>
            -- ==>> FinalInstanceM s0 s1
            <<- Null[Input|RP]TransitionM s0 s1
        Null[Input|RP]TransitionM s1 s2 --< Null[Input|RP]TransitionEndI s1
            -- ==>> Null[Input|RP]TransitionM s0 s2
            <<- Null[Input|RP]TransitionM s0 s1
        [Input|RP]TransitionM s1 symbol s2 --< Null[Input|RP]TransitionEndI s1
            -- (s0, sym, s2> = <s0, s1> + <s1, sym, s2>
            -- ==>> BottomupInstanceM s0 symbol s2
            -- ==>> WorkingRPTransitionM s0 symbol s2
            <<- Null[Input|RP]TransitionM s0 s1
        LeftWantedM s symbol --< InitialRPStateI symbol
            -- ==>> RightWantedM s symbol name ps
            <<- InitialRPStateI symbol name ps
        RightWantedM s symbol name ps --< FinalRPStateI symbol name ps
            -- ==>> RightEndInstanceM s symbol name ps
            <<- FinalRPStateM symbol name ps

        RightWantedM s symbol name ps --< WorkingRPTransitionGroupI symbol name ps
            -- ==>> LeftWantedM s child_next_sym
            <<- WorkingRPTransitionGroupM symbol name ps child_next_sym
        WorkingRPTransitionGroupM sym name ps ch --< WorkingRPTransitionI sym name ps ch
            <<- WorkingRPTransitionM sym name ps ch ps
        RightWantedM s symbol name ps --< LeftWantedI s child_next_sym
            -- ==>> RightWantedM s' symbol name ps'
            <<- LeftWantedM s child_next_sym s'
        RightWantedM s symbol name ps --< 
        LeftNameWantedM s symbol name ps --< InitialRPStateI symbol name
            -- ==>> RightWantedM symbol name ps
            <<- InitialRPStateM symbol name ps
        WaitName symbol
