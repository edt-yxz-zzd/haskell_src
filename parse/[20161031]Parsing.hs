...
esparse (start_syms, rules) tokens = (start_syms', rules') where
    ...
    rules' = sym_syms_ls'
    -------------------
    lookupD = M.findWithDefault []
    -- fill :: MonadState (Map st' [[sym']]) m => st' -> m ()
    fill_st init_st' = do
        st2symss' <- get
        unless (M.member init_st' st2symss') $ do
            let next_sts' = concat $ lslook init_st' st2next_sts'
            forM_ (fmap snd next_sts') fill_st
            st2symss'' <- get
            let symss' = [ next':syms'
                    | (next', st') <- next_sts'
                    , syms' <- concat $ lslook st' st2symss'']
    ...
    fill_sym sym' = do
        ...
        unless (M.member sym' sym2symss') $ do
            let sts' = concat $ lslook sym' sym2sts'
                st2symss'' = execState (fill_sts sts') st2symss'
                symss' = concat $ concat
                        [lslook st' st2symss'' | st' <- sts']
