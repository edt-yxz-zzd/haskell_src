...
type family ContainerType f c a :: *
class (ContainerEx a (ContainerType f c a))
    => FacetContainer f c a where
-- Functor
class (FacetContainer f c a, FacetContainer f c b)
    => OpDynCmap f c a b where
    cmap :: (a -> b) -> ContainerType f c a -> ContainerType f c b
-- Applicative
class   ( OpDynSingleton (ContainerType f c (a->b))
        , OpDynSingleton (ContainerType f c a)
        , OpDynSingleton (ContainerType f c b)
        , FacetContainer f c (a->b)
        , FacetContainer f c a
        , FacetContainer f c b
        )
    => OpDynApp f c a b where
    (<*>), app :: ContainerType f c (a->b) -> ContainerType f c a -> ContainerType f c b

    (*>) :: ContainerType f c a -> ContainerType f c b -> ContainerType f c b
    (<*) :: ContainerType f c a -> ContainerType f c b -> ContainerType f c a



-- Monad
class   ( OpDynSingleton (ContainerType f c b)
        , FacetContainer (ContainerType f c a)
        , FacetContainer (ContainerType f c b)
        )
    => OpDynCat f c a b where
    cat :: ContainerType f c a -> (a -> ContainerType f c b) -> ContainerType f c b
