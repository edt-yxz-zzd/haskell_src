...
show_file fname = do
    ...
    case r of 
        ...
        Right ls -> do
            ...
            let mc_strs = unique_and_show_msgs $ collect_msgs__ls ls
            let ic_strs = unique_and_show_interfaces $ collect_interfaces__ls ls
            print . to_doc $ mc_strs
            print . to_doc $ ic_strs
            let mc2str = ctorstr2map mc_strs
                ic2str = ctorstr2map ic_strs
                icset2strset = S.map (ic2str M.!)
                mcset2strset = S.map (mc2str M.!)
                im2isets' = M.map (S.map icset2strset) $ im2isets cr
                im2isets'' = M.mapKeys (mc2str M.!) im2isets'
                ...
                strsetset2str' = strls2str . fmap strset2str' . S.toList
                im2isets_str = M.map strsetset2str' im2isets''
                to_case_body_str str2str =
                    M.foldrWithKey f "" str2str where
                        f k s b = "    " ++ k ++ " -> " ++ s ++ "\n" ++ b
                im_case_body_str = to_case_body_str im2isets_str
                im_case_str = "return . fmap S.fromList $ case msg of\n"
                                ++ im_case_body_str
            --print . to_doc $ im2isets''
            --print $ to_doc im2isets_str
            putStrLn im_case_str

            let ex2strls = M.map (S.toList . icset2strset) $ ex2iset cr
                ex2strls' = M.mapKeys (mc2str M.!) ex2strls
                ex2ls_str = M.map strls2str ex2strls'
            ...
            let i2outs = M.map (S.toList . mcset2strset) $ i2pushouts cr
                i2outs' = M.mapKeys (ic2str M.!) i2outs
                i2outs_str = M.map strls2str i2outs'
            ...
            let im_i2ex2outs' = im_i2ex2outs cr
                ...
                im_ils_exs2strpair im ils exs =
                    (mcls2str exs, mcls2str outs) where
                    outs = S.toList $ im_i2ex2outs' M.!
                            (im, M.fromList $ zip ils exs)
                im_ils_exs2line im ils exs =
                    let (a, b) = im_ils_exs2strpair im ils exs in
                    a ++ " -> " ++ b ++ "\n"
