...
import Seed.EitherOps (a_E_bEc2aEb_E_c, aEb_E_c2a_E_bEc, either_merge)
...
type A arr r i o = forall s. arr o (Either r s) -> arr i (Either r s)
newtype ArrCPS arr r i o =
    ArrCPS {runArrCPS :: A arr r i o}


_f :: ArrowChoice arr => arr r r' -> A arr r i o -> A arr r' i o
...
change_resultCPS, relabelR_CPS
    :: (ArrowChoice arr, ArrCPS arr ~ ar)
    => arr r r' -> ar r i o -> ar r' i o
change_resultCPS r2t (ArrCPS o2s_to_i2rEs) = ArrCPS (_f r2t o2s_to_i2rEs)

relabelR_CPS = change_resultCPS
{-
instance Boxed (ArrCPS arr r i o) where
    type BoxedFrom (ArrCPS arr r i o) = forall s. arr o s -> arr i (Either r s)
    box = ArrCPS
    unbox = runArrCPS
-}
...
box = ArrCPS
unbox = runArrCPS
unArrCPS :: Arrow arr => ArrCPS arr r i r -> arr i r
unArrCPS a = unbox a (arr Left) >>^ unsafe_unLeft
