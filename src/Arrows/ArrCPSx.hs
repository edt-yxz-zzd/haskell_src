...
---------------------------------------
instance (Arrow arr) => IArrowThrowBaseBy OnExit_Arrow (ArrCPSx arr e) where
    type ArrowExceptionTypeBy OnExit_Arrow (ArrCPSx arr e) = e
instance (Arrow arr) => IArrowThrowBy OnExit_Arrow (ArrCPSx arr e) where
    throwABy_ = box $ const . arr $ Left . snd
