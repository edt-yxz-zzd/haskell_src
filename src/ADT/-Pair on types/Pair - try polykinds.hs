...
type family ToAny (a :: ka) = (r :: kr) where
    ToAny a = a
type family FMapToAny (pa :: *) = (r :: *) where
    FMapToAny (Proxy a) = Proxy (ToAny a)
...
type SwapConstraintP (pa :: ka) = FMapSwap (FMapSwap pa) ~~ pa
type SwapTConstraintP (pca :: kca) = FMapSwapT (FMapSwapT pca) ~~ pca
...
class (IsProxy (FMapSwap _Proxy_a), SwapConstraintP _Proxy_a, IsProxy _Proxy_a)
    => ISwapP (_Proxy_a :: ka) where
    type FMapSwap _Proxy_a :: ksa -- Proxy (Swap a)
class (IsProxy (FMapSwapT _Proxy_ca), SwapTConstraintP _Proxy_ca
        , IsProxy _Proxy_ca)
    => ISwapTP (_Proxy_ca :: kca) where
    type FMapSwapT _Proxy_ca :: ksca -- Proxy (SwapT ca)
...
class (ISwapP pa, ISwapTP (FMapPairT pa)
    ...
    type FMapFst pa :: kf
    type FMapSnd pa :: ks
    type FMapPairT pa :: kt -- sth like constructor
...
to_proxy_pairP :: (IPairP pa) => proxy pa -> (Proxy (FMapFst pa), Proxy (FMapSnd pa))
...
type ProxyProductPair (a :: ka) (b :: kb) = (a :**: b)
...
type instance ToPairP ProxyProductPairT (a) (b)
    = ProxyProductPair a b
instance IPairP (ProxyProductPair a b) where
    type FMapFst (ProxyProductPair a b) = a
    type FMapSnd (ProxyProductPair a b) = b
...
type family ProxyArg (x :: *) = (r :: *) where
    ProxyArg (Proxy a) = a
type family TrueProxyArg (x :: *) = (r :: kr) | r -> x where
