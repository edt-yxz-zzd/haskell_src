...
type family ToPair (t :: Symbol) (f :: Symbol) (s :: Symbol) :: Symbol

--type family SwapK (a :: Symbol) :: *
class (ISwap (Swap a), Swap (Swap a) ~~ a) => ISwap (a :: Symbol) where
    --type Swap a :: SwapK a
    type Swap a :: Symbol
class (ISwapT (SwapT a), SwapT (SwapT a) ~~ a) => ISwapT (a :: Symbol) where
    type SwapT a :: Symbol

type ToPairConstraint (a :: Symbol) = ToPair (PairT a) (Fst a) (Snd a) ~~ a
...
class (IPair (Swap a), ISwap a, ISwapT (PairT a)
    ...
    ) => IPair (a :: Symbol) where
    type Fst a :: Symbol
    type Snd a :: Symbol
    type PairT a :: Symbol -- sth like constructor
