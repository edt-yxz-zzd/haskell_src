...
type Term = String
data Sentences = T Term | N Term [Sentence]
    -- T name - T's only sentences is ([name]:[])
    -- N name sentences - sentences are ([name]:sentences)
data Sentence = [Word]
sA :: () -> Sentences
sA = N (fst s_1) (concat $ transpose [snd s_1, snd s_2])
s_1A :: () -> Sentences
s_1A = N name $ product [[fst a] : snd a, [fst b] : snd b]
