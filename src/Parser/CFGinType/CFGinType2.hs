...
{-
    ...
    class Build by (A := ...)
-}
module Parser.CFGinType.CFGinType2
    ( module Seed.Kind
    , module Seed.FoldAlt
    , ICFG2PlusBy(..)
    , BuildTerminal(..)
...
--------------------------
class (OpArrowPlusBy (CFG2PlusBy by arr cfg n2r) arr
    , OpArrowZeroBy (CFG2PlusBy by arr cfg n2r) arr
    ) => ICFG2PlusBy by arr (cfg :: CFG n) (n2r :: N2R n) where
    type CFG2PlusBy by arr cfg n2r
    type CFG2PlusBy by arr cfg n2r = ByArrowPlus
    cfg_by2plus_by :: proxy '(by, arr, cfg, n2r) -> Proxy (CFG2PlusBy by arr cfg n2r)
    cfg_by2plus_by _ = Proxy
class (ICFG2PlusBy by arr cfg n2r)
    => CFG_Base by arr (cfg :: CFG n) (n2r :: N2R n) where
instance (ICFG2PlusBy by arr cfg n2r)
    => CFG_Base by arr (cfg :: CFG n) (n2r :: N2R n) where
class CFG_Base by arr cfg n2r =>
...
class (CFG_Base by arr cfg n2r, IsUsrAlt usr_alt)
    => BuildRP2Alt by arr
...
class (CFG_Base by arr cfg n2r, BuildRP by arr cfg n2r (Alt2RP alt), IsAlt alt
    ...
    {-# MINIMAL rp2alt #-}
    -}
instance (CFG_Base by arr cfg n2r, BuildRP by arr cfg n2r (Alt2RP alt), IsAlt alt
    , BuildRP2Alt by arr cfg n2r (Alt2UsrAlt alt)
    , LookupLs2Linked
        (RenameSymbols (CFG2Tokens cfg) (UsrAlt2UsrRP (Alt2UsrAlt alt)))
        n2r
...
instance CFG_Base by arr cfg n2r => BuildRP by arr cfg n2r '[] where
    build_rp _ = constA ()
instance (CFG_Base by arr cfg n2r, BuildRP by arr cfg n2r rp
...
instance (CFG_Base by arr cfg n2r, BuildNonTerminal_ by arr cfg n2r alts_of_a a
...
class CFG_Base by arr cfg n2r =>
    ...
    build_nonterminal_
        ...
        -> arr i (LookupE (Left a) n2r)
instance CFG_Base by arr cfg n2r => BuildNonTerminal_ by arr cfg n2r '[] a where
    --build_nonterminal_ _ = zeroArrow
    build_nonterminal_ pa = zeroA where
        plus_by = cfg_by2plus_by $ build_nonterminal__proxy_cast_void pa
        zeroA = zeroBy plus_by
instance (CFG_Base by arr cfg n2r, BuildAlt by arr cfg n2r alt
    ...
    build_nonterminal_ pa = h `plus` t where
...
instance (CFG_Base by arr cfg n2r, BuildNonTerminal by arr cfg n2r a)
    => BuildSymbol by arr cfg n2r (Left a) where
    build_symbol = build_nonterminal . build_symbol__proxy_cast_left
instance (CFG_Base by arr cfg n2r, BuildTerminal by arr cfg n2r a)
...
-----------------------
class (CFG_Base by arr cfg n2r, BuildCFG_ by arr cfg n2r (CFG2AllSymbols cfg)) =>
    BuildCFG by arr (cfg :: CFG n) (n2r :: N2R n)
instance (CFG_Base by arr cfg n2r, BuildCFG_ by arr cfg n2r (CFG2AllSymbols cfg)) =>
    BuildCFG by arr cfg n2r
class CFG_Base by arr cfg n2r =>
    BuildCFG_ by arr (cfg :: CFG n) (n2r :: N2R n) (symbols :: [Symbol n])
instance CFG_Base by arr cfg n2r => BuildCFG_ by arr cfg n2r '[]
instance (CFG_Base by arr cfg n2r, BuildCFG_ by arr cfg n2r ts
    , BuildSymbol by arr cfg n2r h
    ) => BuildCFG_ by arr cfg n2r (h ': ts)


class (CFG_Base by arr cfg n2r, BuildCFG by arr cfg n2r
