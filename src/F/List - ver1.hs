...
-}
--type Exists = forall r. (forall a. a->r) -> r
type Exists r = (forall a. a->r) -> r
...
get :: forall r. (()->r) -> r
get f = f ()
-- type Forall a = forall r. a r
type Pair_ r a b = (a->b->r)->r
type Pair a b = forall r. Pair_ r a b
...
type CBool = forall r. CBool_ r
true, false :: CBool
true = const
false = const id
if_ :: forall a. CBool -> a -> a -> a
if_ = id

type CMaybe_ r a = (a->r) -> r->r
type CMaybe a = forall r. CMaybe_ r a
nothing :: CMaybe a
nothing = const id
just :: forall a. a -> CMaybe a
just a f _ = f a
cmaybe_fmap :: forall a b. (a->b) -> CMaybe a -> CMaybe b
cmaybe_fmap f m = m (\a-> just (f a)) nothing

type CUInt_ r = (Will r->r) -> r->r
type CUInt = forall r. CUInt_ r
cuint_zero :: CUInt
--cuint_succ :: CUInt -> CUInt
cuint_succ :: forall r. CUInt_ r -> CUInt_ r
cuint_zero = const id
...
_msucc :: forall rm ru. Will (CMaybe CUInt) -> CMaybe_CUInt_ rm ru
_msucc m = m () @(CMaybe_CUInt_ rm ru) (\u-> just (cuint_succ u)) (just cuint_zero)
-}
--_msucc :: forall rm ru. Will (CMaybe CUInt) -> (CMaybe_ rm (CUInt_ ru))
type CMaybe_CUInt_ rm ru = CMaybe_ rm (CUInt_ ru)
--_msucc :: forall rm ru. Will (CMaybe CUInt) -> CMaybe_CUInt_ rm ru
--why fail: _msucc :: Will (CMaybe CUInt) -> CMaybe CUInt
_msucc :: forall rm ru. Will (CMaybe_CUInt_ (CMaybe_CUInt_ rm ru) ru) -> CMaybe_CUInt_ rm ru
...
--cuint_pred :: forall rm ru. CUInt_ (CMaybe_CUInt_ rm ru) -> CMaybe_CUInt_ rm ru
cuint_pred u = unTmp_maybe_uint (u tmp_msucc (Tmp_maybe_uint nothing)) where
    ...
    nothing_CUInt :: forall rm ru. CMaybe_CUInt_ rm ru
    nothing_CUInt = 

type CList_ r a = (a-> Will r ->r) -> r->r
type CList a = forall r. CList_ r a
clist_empty :: forall a. CList a
clist_empty = const id
clist_cons :: forall a. a -> CList a -> CList a
